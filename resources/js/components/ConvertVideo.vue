<template>
    <form @submit.prevent="convertFile" v-if="!isConverting">
        <div class="form-group">
            ...
            <input type="file" class="form-control" id="video" v-on:change="files = $event.target.files" multiple>
        </div>
        <div class="form-group">
...
export default {
    ...
    methods: {
        ...
        getFormats() {
            ...
            api.get('/formats/video').then(response => {
        ...
        convertFile() {
            ...
            api.post('/conversions/video', formData, {
                headers: {
        ...
        fetchConversion(sessionId) {
            api.get('/video/' + sessionId + '.zip', { responseType: 'blob' })
                .then(response => {
